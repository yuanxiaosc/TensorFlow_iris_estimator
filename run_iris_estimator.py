...
parser.add_argument('--model_dir', default="model/iris", type=str, help="store model location")

def main(argv):
    args = parser.parse_args(argv[1:])

    # Fetch the data
    (train_x, train_y), (test_x, test_y) = handle_iris_data.load_data()
    ...
    # Train the Model.
    classifier.train(
        input_fn=lambda:handle_iris_data.train_input_fn(train_x, train_y, args.batch_size),
        steps=args.train_steps)

    # Evaluate the model.
    eval_result = classifier.evaluate(
        input_fn=lambda:handle_iris_data.eval_input_fn(test_x, test_y, args.batch_size))
